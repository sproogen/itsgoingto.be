...
const generateHash = (password: string, salt: string): string => crypto.pbkdf2Sync(password, salt, 10000, 512, 'sha512').toString('hex') // eslint-disable-line

export const validatePassword = (user: UserInstance, password: string): boolean => generateHash(password, user.salt) === user.hash // eslint-disable-line
